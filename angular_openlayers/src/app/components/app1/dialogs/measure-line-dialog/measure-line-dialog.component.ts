...
@Component({
    selector: 'app-measure-line-dialog',
    imports: [CommonModule, FormsModule, MatIconModule, MatButtonModule, MatTooltipModule, MatFormFieldModule, MatSelectModule,],
    templateUrl: './measure-line-dialog.component.html',
    styleUrls: ['./measure-line-dialog.component.scss', '../dialogs.component.scss'],
    animations: [
        trigger('slideInOut', [
            state('true', style({
...
export class MeasureLineDialogComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.btnSubscription = this.srv.measureLineBtn$.subscribe(btnValue => {
    ...
    this.draw.on('drawstart', (evt) => {
      ...
      this.geometry = this.sketch.getGeometry().on('change', (evt: { target: any; }) => {
        this.length = getLength(evt.target);
